...
### mrbind: `enum class` causes ODR conflict in generated C++ TU

mrbind's generated `.cpp` `#include`s both the generated C header (`typedef enum Foo {...} Foo`) and the original C++ header (`enum class Foo : int {...}`). These cannot coexist — clang reports "enumeration previously declared as unscoped".

**Fix:** Replace `enum class` types in the mrbind input header with `static const int` / `static const unsigned int` constants. Update all method signatures to use `int`/`unsigned int`. Remove the corresponding `--allow EnumName` lines from `run.sh`.
