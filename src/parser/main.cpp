...
namespace mrbind
{
    ...
    struct VisitorParams
    {
        ...
        // Canonicalize all integer types to fixed-size typedefs.
        bool canonicalize_to_fixed_size_typedefs = false;
        // Canonicalize 64-bit integer types to fixed-size typedefs. This is a subset of `canonicalize_to_fixed_size_typedefs`.
        bool canonicalize_64_to_fixed_size_typedefs = false;
        // Only makes sense if `canonicalize_[64_]to_fixed_size_typedefs` is also true (at least one of the two).
        ...
        if (canonicalize && (params.canonicalize_to_fixed_size_typedefs || params.canonicalize_64_to_fixed_size_typedefs))
        {
            (void)entity.template VisitEachComponent<cppdecl::SimpleType>(
                ...
                [&](cppdecl::SimpleType &simple_type)
                {
                    ...
                    else if (params.canonicalize_to_fixed_size_typedefs && word == "short")
                    {
                        TryApplyTypedef(ci.getTarget().getShortWidth());
                    }
                    else if (params.canonicalize_to_fixed_size_typedefs && word == "int")
                    {
    ...
    struct ClangAstVisitor_Final : clang::RecursiveASTVisitor<ClangAstVisitor_Final>
    {
        ...
        bool VisitFunctionDecl(clang::FunctionDecl *decl) // CRTP override
        {
            ...
            if (decl->isCXXClassMember())
            {
                ...
                ClassMethod *maybe_method = nullptr; // Only gets set if this is a method (as opposed to a constructor).
                ...
                if (basic_func->lifetimes.IsEmpty())
                {
                    // Infer lifetime for `begin`/`end`, and for dereferecing.
                    if (
                        maybe_method &&
                        params->infer_lifetime_iterators &&
                        (
                            maybe_method->name == "begin" ||
                            maybe_method->name == "end" ||
                            // Only unary dereferencing:
                            (maybe_method->name == "operator*" && maybe_method->params.empty()) // No different than `IsCallableWithNumArgs()`, since overloaded operators can't have default arguments.
                        )
                    )
                    {
                        bool types_ok = true;
            ...
            // This only runs if there is no existing lifetime information (from attributes).
            if (
                new_func.lifetimes.IsEmpty() &&
                params->infer_lifetime_iterators && (new_func.name == "begin" || new_func.name == "end" || new_func.name == "operator*") && new_func.IsCallableWithNumArgs(1))
            {
                bool types_ok = true;
                if (params->enable_cppdecl_processing)
