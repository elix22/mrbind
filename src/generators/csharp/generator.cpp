...
namespace mrbind::CSharp
{
    ...
    std::string Generator::WrapForExceptionHandling(std::string str, bool can_throw)
    {
        assert(!str.empty() && str.ends_with('\n'));

        assert(str.find("return ") == std::string::npos); // Shouldn't wrap return statements!

        if (!can_throw || !c_desc.exception_handling_enabled)
            return str;

        return
            ExceptionHandlingPre() +
            str +
            ExceptionHandlingPost();
    }

    std::string Generator::ExceptionHandlingPre(bool can_throw)
    {
        if (can_throw)
            return RequestHelper("_Exceptions") + ".Prepare();\n";
        else
            return "";
    }

    std::string Generator::ExceptionHandlingPost(bool can_throw)
    {
        if (can_throw)
            return RequestHelper("_Exceptions") + ".ThrowIfNeeded();\n";
        else
            return "";
    }
    ...
    std::string Generator::TypeBindingFlagsToString(TypeBindingFlags flags)
    {
        ...
        ret += bool(flags & TypeBindingFlags::try_enable_sugar) ? " (with sugar enabled if possible)" : "";
        ret += bool(flags & TypeBindingFlags::try_enable_sugar_for_param_usage) ? " (with sugar enabled if supported for parameter usage)" : "";
        ret += bool(flags & TypeBindingFlags::try_enable_sugar_for_return_usage) ? " (with sugar enabled if supported for return usage)" : "";
        ...
        ret += bool(flags & TypeBindingFlags::treat_rvalue_refs_as_lvalue_refs) ? " (treating rvalue refs as lvalue refs)" : "";
        ret += bool(flags & TypeBindingFlags::avoid_post_statements) ? " (avoiding post statements)" : "";
        return ret;
    }
    ...
    {
        // Handle `try_enable_sugar`.
        if (bool(flags & TypeBindingFlags::try_enable_sugar))
        {
            const bool want_param = (flags & TypeBindingFlags::try_enable_sugar_for_param_usage) == TypeBindingFlags::try_enable_sugar_for_param_usage;
            const bool want_return = (flags & TypeBindingFlags::try_enable_sugar_for_return_usage) == TypeBindingFlags::try_enable_sugar_for_return_usage;

            flags |= TypeBindingFlags::enable_sugar;
            flags &= ~TypeBindingFlags::try_enable_sugar & ~TypeBindingFlags::try_enable_sugar_for_param_usage & ~TypeBindingFlags::try_enable_sugar_for_return_usage;
            if (auto opt = GetTypeBindingOpt(cpp_type, flags); opt && (!want_param || opt->param_usage) && (!want_return || opt->return_usage))
                return opt; // Found a sugared binding.

            // No sugar, proceed normally.
            flags &= ~TypeBindingFlags::enable_sugar;
        }

        // Adjust the flags before doing anything else.
        ...
        try
        {
            // Note that if you're checking for rvalue references anywhere in this function, that should be guarded by this variable as a condition.
            bool allow_rvalue_refs = !bool(flags & TypeBindingFlags::treat_rvalue_refs_as_lvalue_refs);
            ...
            auto MaybePrependRvalueTag = [
                this,
                is_rvalue_ref = allow_rvalue_refs && cpp_type.Is<cppdecl::Reference>() && cpp_type.As<cppdecl::Reference>()->kind == cppdecl::RefQualifier::rvalue
            ...
            if (!bool(flags & TypeBindingFlags::enable_sugar))
            {
                                ...
                                switch (elem.kind)
                                {
                                  ...
                                  case CInterop::ClassKind::uses_pass_by_enum:
                                    {
                                        ...
                                        return CreateBinding({
                                            .param_usage = TypeBinding::ParamUsage{
                                                .make_strings = [this, by_value_helper, csharp_underlying_ptr_type, csharp_underlying_ptr, cpp_param_never_throws](const std::string &name, bool /*have_useless_defarg*/)
                                                {
                                                    return TypeBinding::ParamUsage::Strings{
                                                        .cpp_never_throws = cpp_param_never_throws,
                                            ...
                                            .return_usage = TypeBinding::ReturnUsage{
                                                .cpp_never_throws = false, // Because of the heap allocation.
                                  ...
                                  case CInterop::ClassKind::trivial_via_ptr:
                                    return CreateBinding({
                                        ...
                                        .return_usage = TypeBinding::ReturnUsage{
                                            .cpp_never_throws = false, // Heap allocation!
                                            .dllimport_return_type = csharp_underlying_ptr_type,
                ...
                else
                {
                    ...
                    {
                        const bool is_rvalue_ref = allow_rvalue_refs && cpp_type.As<cppdecl::Reference>()->kind == cppdecl::RefQualifier::rvalue;
                                ...
                                [&](const CInterop::TypeKinds::Class &elem) -> const TypeBinding *
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        return CreateBinding({
                                            ...
                                            .return_usage = TypeBinding::ReturnUsage{
                                                .cpp_never_throws = true, // Even if the C# `new T(...)` throws, this isn't our problem.
                                                .needs_temporary_variable = true,
                                                .dllimport_return_type = csharp_underlying_ptr_type,
                                                .csharp_return_type = csharp_type + "?",
                                                .can_be_kept_alive = true,
            ...
            else
            {
                ...
                auto TypeMatchesExactlyOrConstOrRvalueRef = [&](const cppdecl::QualifiedName &target_name, cppdecl::QualifiedName::EqualsFlags eq_flags = {}) -> bool
                {
                    ...
                    if (!cpp_type.IsConst(1) && cpp_type.Is<cppdecl::Reference>() && (!allow_rvalue_refs || cpp_type.As<cppdecl::Reference>()->kind == cppdecl::RefQualifier::lvalue))
                    ...
                    if (elem_binding->param_usage_with_default_arg)
                    {
                        ...
                        ](const std::string &name, bool have_useless_defarg)
                        {
                            ...
                            if (is_class_with_pass_by_enum)
                            {
                                ...
                                for (auto &elem : strings.dllimport_args)
                                    elem = Strings::Replace(elem, pass_by_old, pass_by_new);
                            }
                ...
                auto MakeStringLikeReadOnlyBinding = [&] -> TypeBinding
                {
                    ...
                    return {
                        .param_usage = TypeBinding::ParamUsage{
                            ...
                            {
                                return TypeBinding::ParamUsage::Strings{
                                    .cpp_never_throws = false, // Allocating a string might fail.
                                ...
                                return TypeBinding::ParamUsage::Strings{
                                    ...
                                    .dllimport_args =
                                        (
                                            ...
                                            :
                                                std::vector<std::string>{name + " is not null ? __ptr_" + name + " : null", name + " is not null ? __ptr_" + name + " + __len_" + name + " : null"}
                                        ),
                                    .scope_close = "}\n",
                                };
                            },
    ...
    Generator::ExtraClassContents Generator::GetExtraContentsForParsedClass(const cppdecl::QualifiedName &cpp_name, std::optional<bool> class_part_kind)
    {
        ...
        // `std::function`. This where we interact with C# callables.

        static const cppdecl::QualifiedName cpp_name_std_function = cppdecl::QualifiedName{}.AddPart("std").AddPart("function");

        if (
            cpp_name.Equals(cpp_name_std_function, cppdecl::QualifiedName::EqualsFlags::allow_missing_final_template_args_in_target)
        )
        {
            try
            {
                if (cpp_name.parts.back().template_args.value().args.size() != 1)
                    throw std::runtime_error("Expected `std::function<...>` to have exactly one template argument.");
                if (!cpp_name.parts.back().template_args.value().args.front().IsType())
                    throw std::logic_error("Internal error: The template argument of `" + CppdeclToCode(cpp_name) + "` doesn't parse as a type.");
                if (!cpp_name.parts.back().template_args.value().args.front().AsType()->Is<cppdecl::Function>())
                    throw std::logic_error("Internal error: The template argument of `" + CppdeclToCode(cpp_name) + "` is not a function type.");
                ...
                // Determine the (non-output) DllImport parameters, and the C# parameters.

                struct CSharpParamInfo
                {
                    const TypeBinding::ReturnUsage *ret_usage = nullptr;
                    TypeBinding::ParamUsage::Strings::CSharpParam strings;
                };

                std::vector<CSharpParamInfo> csharp_params;

                for (const auto &cpp_param_decl : cpp_name.parts.back().template_args.value().args.front().AsType()->As<cppdecl::Function>()->params)
                {
                    ...
                    auto &new_csharp_param = csharp_params.emplace_back();
                    new_csharp_param.ret_usage = &return_usage.value();
                    new_csharp_param.strings.type = return_usage->csharp_return_type;
                    new_csharp_param.strings.name = "_" + std::to_string(csharp_params.size()); // 1-based indices.

                    if (new_csharp_param.strings.type.find('*') != std::string::npos)
                        csharp_delegate_unsafe = true;
                ...
                // Write the C# delegate type.
                if (class_part_kind.value())
                {
                    WriteSeparator();
                    ret.text +=
                        "public " +
                        std::string(csharp_delegate_unsafe ? "unsafe " : "") +
                        "delegate " +
                        csharp_return_type +
                        (csharp_return_type.ends_with('*') ? "" : " ") +
                        "Delegate(";

                    bool first = true;
                    for (const auto &param : csharp_params)
                    {
                        if (!std::exchange(first, false))
                            ret.text += ", ";
                        ret.text += param.strings.ToString();
                    }

                    ret.text += ");\n";
                }

                // Write the dll-importable (C-style) delegate type.
                if (class_part_kind.value())
                {
                    WriteSeparator();
                    ret.text +=
                    ...
                    bool first = true;
                    for (const auto &param : csharp_dllimport_params)
                    {
                        if (!std::exchange(first, false))
                            ret.text += ", ";
                        ret.text += param.ToString();
                    }

                    ret.text += ");\n";
                }

                // Write the wrapper function that's passed into the dll-importable delegate.
                if (class_part_kind.value())
                {
                    WriteSeparator();
                    ret.text +=
                    ...
                    { // Assemble the function body.
                        // Convert C parameters to the C# parameters.
                        std::string body;
                        std::string csharp_args;
                        for (int i = 0; const auto &param : csharp_params)
                        {
                            // Create a variable for this parameter.
                            // In case you start wondering, why don't we use immediately-invoked lambdas, the answer is that C# doesn't let you immediately-invoke them. Without first assigning them to a delegate, I assume.
                            // And even if that worked, I'm not 100% sure I trust the C# compiler to optimize them out correctly.

                            // 1-based for user-friendliness.
                            // The names `_i` are reserved for the C-style parameters, so this has to be something else.
                            std::string var_name = "_arg_" + std::to_string(i + 1);

                            // Append to the C# argument list.
                            if (i != 0)
                                csharp_args += ", ";

                            if (param.strings.type.starts_with("ref readonly "))
                                csharp_args += "in ";
                            else if (param.strings.type.starts_with("ref "))
                                csharp_args += "ref ";

                            csharp_args += var_name;

                            // Declare the variable.
                            body +=
                                param.strings.type +
                                (param.strings.type.ends_with('*') ? "" : " ") +
                                var_name;

                            // Reference variables need a dummy initializer.
                            ...
                            // Lastly, assign to the variable.
                            // It's a bit weird that `param.strings.name` represents the C-style parameter name here, but whatever. This looks better in the C#-style delegate, I guess.
                            body += param.ret_usage->MakeReturnStatements(var_name + " =", param.strings.name) + '\n';

                            i++;
                        }

                        // Call the function. If it returns non-void, save it result into a variable.
                        if (csharp_return_type != "void")
                            body += csharp_return_type + " " + csharp_ret_var_name + " = "; // Can't use `var` here, as that apparently strips `ref`. But even if it didn't, I prefer to spell the type explicitly for clarity.
                        if (csharp_return_type.starts_with("ref "))
                            body += "ref ";
                        ...
                        // Write the resulting body, possibly with additional exception handling.
                        if (c_desc.exception_handling_enabled)
                        {
                            auto dllimport_throw = MakeDllImportDecl(GetFilePlaceholderForCHelpers(), c_desc.helpers_prefix + "ThrowExceptionOnCallbackExit", "void", "byte *message");

                            ret.text += Strings::Indent(
                                "try\n"
                                "{\n" +
                                Strings::Indent(body) +
                                "}\n"
                                ...
                                Strings::Indent(
                                    ...
                                    "{\n"
                                    "    " + dllimport_throw.csharp_name + "(__ex_bytes_ptr);\n"
                                    "}\n" +
                                    (csharp_dllimport_return_type == "void" ? "" : "return default;\n") // Need something to silence the warning.
                                ) +
                                "}\n"
                            );
                        }
            ...
            {
                std::throw_with_nested(std::runtime_error("While generating the delegate logic for `" + CppdeclToCode(cpp_name) + "`:"));
            }
        }
    ...
    std::vector<Generator::EmitVariant> Generator::GetMethodVariants(const CInterop::ClassMethod &method)
    {
        ...
        if (auto *ctor = std::get_if<CInterop::MethodKinds::Constructor>(&method.var))
        {
            ...
            if (ctor->is_copying_ctor)
            {
                ...
                if (std::get<CInterop::TypeKinds::Class>(desc.var).kind == CInterop::ClassKind::uses_pass_by_enum && bool(desc.traits->copy_constructible))
                ...
                if (bool(desc.traits->copy_constructible) && !desc.traits.value().copy_constructor_takes_nonconst_ref)
    ...
    try
        ...
        must_handle_exceptions(generator.c_desc.exception_handling_enabled && !func_like.is_noexcept),
        ...
        acts_on_copy_of_this(is_incr_or_decr && emit_variant == EmitVariant::static_incr_or_decr)
    {
        { // Find the return type binding.
                ...
                ? (
                    ...
                    (
                        ...
                        [&]{
                            ...
                            return bool(traits.copy_constructible) && !bool(traits.move_constructible);
                        }()
                    )
            ...
            if (!ret_binding->cpp_never_throws && generator.c_desc.exception_handling_enabled)
                    ...
                    else
                    {
                        ...
                        if (!param_type.Is<cppdecl::Reference>())
                        {
                            ...
                            if (class_desc.kind == CInterop::ClassKind::uses_pass_by_enum)
                            {
                                ...
                                assert(bool(type_desc.traits.value().copy_constructible)); // This is strictly copyable at this point.
    ...
    void Generator::FuncLikeEmitter::Emit(OutputFile &file, std::optional<ShadowingDesc> shadowing_desc)
    {
        try
        {
            assert(must_handle_exceptions <=/*implies*/ generator.c_desc.exception_handling_enabled);
            ...
            {
                extra_statements = generator.ExceptionHandlingPre() + extra_statements;
                extra_statements_after += generator.ExceptionHandlingPost();
            }
            ...
            { // Call the imported function.
                ...
                { // Add the argument list.
                    ...
                    for (const auto &param : param_strings)
                    {
                        for (const auto &arg : param.dllimport_args)
                        {
                            if (first)
                                first = false;
                            else
                ...
                auto MakeReturnStatements = [&](const std::string &expr, bool *c_returns_void = nullptr) -> std::string
                {
                    if (c_returns_void)
                        *c_returns_void = false;

                    auto StringLooksLikeReturnedVoid = [&](std::string_view str)
                    {
                        return str.starts_with(expr + ";"); // Need `starts_with()` instead of `==` to support tag types, which produce `expr; return new();`. Not supporting those results in C# compilation errors.
                    };
                    ...
                    if (force_void_return_type || acts_on_copy_of_this || need_result_in_variable_for_lifetimes)
                    {
                        ...
                        if (c_returns_void && StringLooksLikeReturnedVoid(ret_statements))
                            *c_returns_void = true;
                        ret += ret_statements;
                        ret += '\n';
                ...
                if (is_ctor)
                {
                    ...
                    // `--fat-objects` initialization. (Those should catch their own exceptions, so it's fine to have them after `extra_statements_after`.)
                    if (!in_exposed_struct)
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        if (class_kind == CInterop::ClassKind::uses_pass_by_enum)
                        {
                            ...
                            assert(bool(c_desc.FindTypeOpt(CppdeclToCode(this_type))->traits->copy_constructible));
    ...
    std::optional<Generator::ShouldEmitResult> Generator::ShouldEmitMethod(const cppdecl::QualifiedName &class_name, const CInterop::TypeDesc &type_desc, const CInterop::ClassMethod &method, EmitVariant emit_variant)
    {
        ...
        bool should_emit = std::visit(Overload{
            ...
            [&](const CInterop::MethodKinds::Constructor &elem)
            {
                ...
                (void)elem;
                return bool(type_desc.traits.value().destructible);
            },
                ...
                try
                {
                    ...
                    { // Write the bases.
                        ...
                        else
                        {
                            ...
                            if (bool(type_desc.traits.value().destructible))
                            {
                                BaseSeparator();
                    ...
                    if (!is_exposed_struct_by_value && IsConst())
                    {
                        ...
                        else
                        {
                            ...
                            { // Check if the underlying shared pointer isn't null.
                                file.WriteSeparatingNewline();
                                WriteComment(file,
                    ...
                    if (!is_exposed_struct_by_value && IsConst() && (bool(type_desc.traits.value().destructible) || shared_ptr_desc))
                    {
                    ...
                    if (!is_exposed_struct_by_value && shared_ptr_desc)
                    {
                        ...
                        if (IsConst())
                        {
                            ...
                            {
                                file.WriteString(
                                    WrapForExceptionHandling("var ret = new(" + dllimport_construct_aliasing.csharp_name + "(" + RequestHelper("_PassBy") + ".copy, ownership, ptr), is_owning: true);\n") +
                                    "return ret;\n"
                                );
                            }
                            else
                    ...
                    if (class_desc.kind == CInterop::ClassKind::exposed_struct) // Sic, not `in_exposed_struct`, see above.
                    {
                        ...
                        if (!is_exposed_struct_by_value && type_desc.traits.value().default_constructible == CInterop::SpecialMemberKind::trivial)
                        {
                            file.WriteSeparatingNewline();
            ...
            { // Emit the parameter passing helper classes. Those can't be generic because we need to copy our conversion operators to them.
                ...
                { // `_ByValue_...`.
                    auto EmitByValueHelper = [&](bool is_opt_opt, bool is_shared)
                    {
                        ...
                        const bool allow_def_ctor = is_shared || bool(type_desc.traits.value().default_constructible);
                        const bool allow_copy_ctor = is_shared || bool(type_desc.traits.value().copy_constructible);
                        const bool allow_move_ctor = is_shared || bool(type_desc.traits.value().move_constructible);
    ...
    void Generator::Generate()
    {
        ...
        { // Collect `begin()` and `end()` functions.
            ...
            for (const auto &func : c_desc.functions)
            {
                ...
                if (!param_type.Is<cppdecl::Reference>() && !bool(param_type_desc->traits->copy_constructible))
                    continue;
            ...
            for (const auto &[type_name, type] : c_desc.cpp_types.Map())
            {
                ...
                for (const CInterop::ClassMethod &method : cl->methods)
                {
                    ...
                    // If the parameter is by value, the class must be copyable.
                    if (!param_type.Is<cppdecl::Reference>() && !bool(type.traits->copy_constructible))
                        continue;
            ...
            if (!requested_helpers.empty())
            {
                ...
                if (requested_helpers.erase("_Alloc"))
                {
                    ...
                    if (c_desc.exception_handling_enabled)
                    {
                        file.WriteString(
                            WrapForExceptionHandling(
                                "var ret = " + dllimport_alloc.csharp_name + "(size);\n"
                            ) +
                            "return ret;\n"
                        );
                    }
                ...
                if (requested_helpers.erase("_Exceptions"))
                {
                    auto dllimport_set_simple_handler = MakeDllImportDecl(GetFilePlaceholderForCHelpers(), c_desc.helpers_prefix + "SetSimpleExceptionHandler", "void", "HandlerDelegate func");
                ...
                { // Custom exceptions.
                    ...
                    CreateExceptionClassIfNeeded("NativeException", "Exception", "/// A C++ exception propagated to C#.\n");
